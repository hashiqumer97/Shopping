...
function LoadProducts() {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (oj !== null) {
                document.getElementById("desc").innerText = oj.productDescription;
                document.getElementById("price").innerText = oj.unitPrice;
            }
...
function confirmOrder() {
    ...
    http.open("POST", "../AddCart/CreateOrders", true);
...
function deleteEntireOrder() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function () {
            ...
            console.log(JSON.stringify(orders));
            http.open("POST", "../Orders/DeleteOrders", true);
            http.setRequestHeader("Content-Type", "application/json");
            http.send(JSON.stringify(orders));
